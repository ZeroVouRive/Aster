...
def main(args):
 ...
 with sync_playwright() as p:
  ...
  try:
   ...
   def revokedstream():
    frame.locator('#text').fill('Original before revocation');assert frame.locator('#text').input_value()=='Original before revocation';frame.locator('#save').click();d=picker('Save to Aster');d.get_by_label('File name',exact=True).fill('revoked.txt');d.get_by_role('button',name='Save',exact=True).click();assert not done()['error'];frame.evaluate('async()=>{window.stream=await h.createWritable();await stream.write("must not commit");}');js("OS.webIO.revoke('io-fixture');");result=frame.evaluate('async()=>{try{await stream.close();return "unsafe"}catch(e){return e.name}}');assert result=='InvalidStateError',result;js("assert(await OS.fs.text(await OS.fs.read('/Documents/IO/revoked.txt'))==='Original before revocation','Revocation must preserve the committed original bytes');")
