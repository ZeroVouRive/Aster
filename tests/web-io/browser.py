 ...
 with sync_playwright() as p:
  ...
  try:
   ...
   def appdrop():
    ...
    page.mouse.move(source['x']+20,source['y']+8);page.mouse.down();page.mouse.move(target['x']+target['width']-30,target['y']+target['height']-30,steps=30);page.mouse.up();page.locator('.file-row[data-path="/Documents/IO/drag.bin"]').wait_for();js("assert(JSON.stringify([...new Uint8Array(await(await OS.fs.blob(await OS.fs.read('/Documents/IO/drag.bin'))).arrayBuffer())])==='[0,255,128,13,10]');")
