...
def run(page, ctx, js, picker, frame, done, check, args, root, origin):
    ...
    def storage_collision():
        js("await OS.webIO.update('io-fixture','storage',true);await OS.fs.write('/Documents/App storage/io-fixture','not a directory');window.collisionSession=OS.webIO.makeSession(iow,iow.body.querySelector('iframe'),'io-fixture');window.storageCollision=OS.webIO.dispatch(collisionSession,'storage',{}).then(()=>null,e=>e.name);")
        ...
        js("assert(collisionSession.caps.size===0);assert(await OS.fs.text(await OS.fs.read('/Documents/App storage/io-fixture'))==='not a directory');await OS.fs.remove('/Documents/App storage/io-fixture',true);await OS.webIO.update('io-fixture','storage',null);")
