...
def main(args):
    ...
    with sync_playwright() as p:
        ...
        try:
            boot()
            check('Boot registers all ten workflows and preserves 69 web apps',lambda:js("assert([...OS.apps.values()].filter(a=>!a.webApp).length===28);assert(OS.webCatalog.apps.length===69);for(const id of ['files','archives','clipboard','focus','widgets','workspaces','history','storage','accessibility','recorder'])assert(OS.apps.has(id));assert(!document.querySelector('.web-app-frame'));return {apps:OS.apps.size,storage:OS.db.mode};"))
            def explorer():
