...
def main(args):
    ...
    with sync_playwright() as p:
        ...
        def web_index():start();page.get_by_role('button',name='Web apps, 74 projects',exact=True).click()
        ...
        try:
            ...
            if args.live:
                ...
                for app in apps:
                    ...
                    try:
                        ...
                        if app['repo'] in ['PaintXP','Vellum','Gridline','AxiomCAD','VeyraWorkspace','AsterionEDA','TwinForge','Branchglass','NotepadXP','Formalyth','Jailbreak']:page.screenshot(path=str(out/(app['repo']+'-live.png')))
            ...
            def search():
                ...
                page.wait_for_function('document.querySelectorAll(".start-main .search-result").length===9')
                assert page.locator('.start-main').inner_text().count('CAD & Manufacturing')==9
            ...
            def mobile():
                ...
                page.screenshot(path=str(out/'window-mobile-fixture.png'));close_all();page.evaluate('window.webResizeBack=false;window.addEventListener("resize",()=>window.webResizeBack=true,{once:true})');page.set_viewport_size({'width':1440,'height':1000});page.wait_for_function('window.webResizeBack && innerWidth===1440')
            ...
            def requested_apps():
                for repo,title,category in [('VeyraWorkspace','Veyra Workspace','office'),('AsterionEDA','Asterion EDA','cad'),('TwinForge','TwinForge','office'),('Branchglass','Branchglass','development'),('NotepadXP','Notepad XP','office'),('Formalyth','Formalyth','cad'),('Jailbreak','Jailbreak','development')]:
            ...
            def standalone():
                boot(True);assert page.evaluate('Aster.webCatalog.apps.length')==74
